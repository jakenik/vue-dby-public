...
const WxApi = class WxApi {
  ...
  wxConfig ({ source, jsApiList, debug, succ, fail }) {
    const app = this.app
    if (!app[source]) return $logger.error('当前无匹配的JSSDK验证')
    ...
    let cSucc = res => {
      ...
      wx.ready(function () {
        $logger.info('JSSDK初始化成功')
        succ(res)
      })
      wx.error(function (res) {
        $logger.error('JSSDK初始化失败', res)
        fail(res)
        // config信息验证失败会执行error函数，如签名过期导致验证失败，具体错误信息可以打开config的debug模式查看，也可以在返回的res参数中查看，对于SPA可以在这里更新签名。
      })
    }
    let cFail = res => {
      $logger.error('wxConfig 出错', res)
    }
    $request.getToken({
      succ: () => {
        $request.httpRequest({
  ...
  wxPageInitShare ({ path, succ, fail }) {
    let cSucc = res => {
      ...
      const data = {
        ...
        succ: res => {
          succ(res)
          $logger.info('用户页面分享成功', res)
        },
        fail: res => {
          fail(res)
          $logger.error('用户页面分享取消', res)
        }
      }
      this.wxShare(data)
    }
    let cFail = res => {
      $logger.error('获取JSSDK页面初始化分享失败', res)
    }
    if (this.initShare) {
    ...
    wx.stopRecord({
      success: function (res) {
        ...
        if (jetLag < voiceMinTime) {
          fail({minTime})
          $logger.info('录音时间不能小于' + (minTime / 1000) + '秒')
          return
        }
